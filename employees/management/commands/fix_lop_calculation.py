...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
                ...
                if current_lop != expected_lop:
                    ...
                    if not dry_run:
                        with transaction.atomic():
                            ...
                            balance.save()
                        self.stdout.write(
                            self.style.SUCCESS(f"  ✅ Fixed LOP for {employee.user.get_full_name()}")
                        )
                    else:
                        self.stdout.write(
                            self.style.WARNING(f"  Would fix LOP for {employee.user.get_full_name()}")
                        )
        ...
        if fixed_count == 0:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"✅ {action} LOP calculation for {fixed_count} employees")
            )
